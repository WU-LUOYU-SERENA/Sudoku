...
class Sudoku(object):
    ...
    def isConsistent(self, currAns):
        ...
        for j in range(9):
            ...
            if not len(rowSet) == 9 - totalZeros: # the column contains duplicates
                return False
        ...
        for x in range(3):
            for y in range(3):
                squareSet = set()
                totalZeros = 0
                for i in range(x * 3 + 3):
                    for j in range (y * 3 + 3):
                        if currAns[i][j] == 0:
                            totalZeros += 1
                        else:
    ...
    def backTrackingSearch(self, currAns):
        ...
        for value in range(1, 10):
            currAns[i][j] = value
            result = self.backTrackingSearch(currAns)
            if not result == False:
                return result
            currAns[i][j] = 0
        return False
    ...
    def solve(self):
        return self.backTrackingSearch(self.ans)
        #return self.ans
